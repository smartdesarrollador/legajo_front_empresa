...
export function contratoIndeterminadoDocx(
  ...
  try {
    ...
    const clausulas = [
      ...
      new Paragraph({
        children: [
          ...
          new TextRun(
            ', bajo subordinación y de manera personal; de conformidad con lo establecido por el artículo 9 del Texto Único Ordenado del Decreto Legislativo N° 728 – Ley de Productividad y Competitividad Laboral aprobado por el Decreto Supremo N° 003-97-TR y su Reglamento.\n\n'
          ),
        ],
      }),
      // Lugar de prestación de servicios
      new Paragraph({
        text: `CLÁUSULA ${num_valores[3]}. - DEL LUGAR DE PRESTACIÓN DE SERVICIOS`,
        heading: HeadingLevel.HEADING_2,
        spacing: { before: 200, after: 200 },
      }),
      new Paragraph({
        children: [
          new TextRun('Las partes acuerdan que, en atención al cargo de '),
          new TextRun({ text: datosLocales.oferta_laboral, bold: true }),
          new TextRun(
          ...
          new TextRun({ text: registroEmpleador.domiciliado, bold: true }),
          new TextRun('.\n\n'),
        ],
      }),
    ...
    console.error('Error en contratoIndeterminadoDocx:', error);
    throw error;
  }
}
