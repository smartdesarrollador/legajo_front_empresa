...
export function contratoIntermitenteDocx(
  ...
  try {
    ...
    const introduccion = new Paragraph({
      children: [
        new TextRun({
          text: 'Conste por el presente documento, el Contrato Individual de Trabajo bajo la modalidad de Intermitente que celebran, de conformidad con lo establecido en el artículo 64 del Texto Único Ordenado del Decreto Legislativo 728 – Ley de Productividad y Competitividad Laboral aprobado por el Decreto Supremo N.º 003-97-TR, de una parte,\n\n',
        }),
      ],
    });
    ...
    const clausulas = [
      ...
      new Paragraph({
        children: [
          new TextRun('EL EMPLEADOR se dedica a las actividades de '),
          new TextRun({
            text: registroEmpleador.actividad_economica,
            bold: true,
          }),
          new TextRun(' motivo por el que requiere contratar a un '),
          new TextRun({
    ...
    console.error('Error en contratoIntermitenteDocx:', error);
    throw error;
  }
}
